...
router.post('/stripe/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  try {
    ...
    async function processAffiliateCommission({ referredUserId, planAmount }) {
      ...
      try {
        ...
        const planValue = Number(planAmount) || 0;
        if (planValue <= 0) {
          await client.query('ROLLBACK');
          return;
        }
        ...
        const affiliateSubResult = await client.query(
        ...
        const affiliateSub = affiliateSubResult.rows[0];
        const affiliateStatus = String(affiliateSub?.status || '').toLowerCase();
        const affiliateEndDate = affiliateSub?.end_date ? new Date(affiliateSub.end_date) : null;
        const affiliateActive =
          affiliateStatus === 'active' &&
          (!affiliateEndDate || !Number.isFinite(affiliateEndDate.getTime()) || affiliateEndDate.getTime() > Date.now());

        const settingsResult = await client.query(
          `SELECT key, value
           FROM settings
           WHERE user_id IS NULL
             AND key IN ('affiliateDefaultCommission')`
        );
        const settingsMap = {};
        settingsResult.rows.forEach((row) => {
          settingsMap[row.key] = row.value;
        });
        const globalCommission = Number(String(settingsMap.affiliateDefaultCommission || '').replace(',', '.')) || 25;

        await client.query(
          `INSERT INTO affiliates_profiles (user_id)
           VALUES ($1)
        ...
        const customCommissionRaw = profileResult.rows[0]?.custom_commission;
        const customCommission =
          customCommissionRaw === null || customCommissionRaw === undefined
            ? null
            : Number(String(customCommissionRaw).replace(',', '.'));
        const percentage = Number.isFinite(customCommission) ? customCommission : globalCommission;

        const commissionValue = Number((planValue * (percentage / 100)).toFixed(2));
          ...
          await client.query(
            `UPDATE affiliates_profiles
             SET total_earned = total_earned + $1,
                 ...
                 updated_at = NOW()
             WHERE user_id = $2`,
            [commissionValue, affiliateId]
          );
          await client.query(
            `UPDATE referrals
             SET commission_value = $1,
                 status = 'paid',
                 paid_at = NOW()
             WHERE id = $2`,
            [commissionValue, referral.id]
          );
        } else {
          await client.query(
            `UPDATE referrals
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (userId) {
        try {
          ...
          const amountFromMetadata = Number(session.metadata?.price_cents || 0) / 100;
          const amountFromSession = typeof session.amount_total === 'number' ? session.amount_total / 100 : 0;
          const amount = amountFromMetadata || amountFromSession || 0;
          ...
          try {
            await processAffiliateCommission({ referredUserId: parseInt(userId, 10), planAmount: amount });
          } catch (affiliateErr) {
            console.error('[WEBHOOK] Affiliate commission error:', affiliateErr?.message || affiliateErr);
          }
        } catch (err) {
