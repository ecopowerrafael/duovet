...
const registerSchema = Joi.object({
  ...
  ref: Joi.alternatives().try(Joi.number().integer(), Joi.string()).optional()
});
...
router.post('/register', async (req, res, next) => {
  ...
  const refRaw = req.query?.ref ?? req.body?.ref;
  try {
    ...
    const refId = Number.isFinite(parseInt(refRaw, 10)) ? parseInt(refRaw, 10) : null;
    if (newUserId && refId && refId !== newUserId) {
      try {
        const affiliateExists = await db.query('SELECT id FROM users WHERE id = $1', [refId]);
        if (affiliateExists.rows.length > 0) {
          ...
          await db.query(
            `INSERT INTO referrals (affiliate_id, referred_user_id, status, referred_at, created_at)
...
router.post('/login', async (req, res, next) => {
  ...
  try {
    ...
    if (status === 'trial' && isTrialExpired(user.created_at)) {
      status = 'expired';
...
function shouldAllowExpiredRequest(req) {
  ...
  if (url.startsWith('/api/affiliates')) return true;
