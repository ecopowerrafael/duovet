...
async function migrateDatabase() {
  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS affiliates_profiles (
        user_id INTEGER PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
        custom_commission NUMERIC(5, 2),
        pix_key TEXT,
        total_earned NUMERIC(12, 2) DEFAULT 0,
        balance_available NUMERIC(12, 2) DEFAULT 0,
        balance_reserved NUMERIC(12, 2) DEFAULT 0,
        created_at TIMESTAMP DEFAULT NOW(),
        updated_at TIMESTAMP DEFAULT NOW()
      );
    `);
    ...
    `);
    await client.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_referrals_referred_user_unique ON referrals(referred_user_id)`);
    ...
    `);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_payout_requests_user_id ON payout_requests(user_id)`);
