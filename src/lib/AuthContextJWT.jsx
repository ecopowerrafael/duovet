...
import { auth as firebaseAuth, googleProvider, getGoogleAccessTokenFromResult } from './firebase';
...
export const AuthProvider = ({ children }) => {
  ...
  const loginWithGoogle = async () => {
    ...
    try {
      ...
      // Captura accessToken e refreshToken do Google
      const accessToken = getGoogleAccessTokenFromResult(result);
      let refreshToken = null;
      if (result._tokenResponse && result._tokenResponse.refreshToken) {
        refreshToken = result._tokenResponse.refreshToken;
      } else if (result.user && result.user.stsTokenManager && result.user.stsTokenManager.refreshToken) {
        refreshToken = result.user.stsTokenManager.refreshToken;
      }

      // Envia tokens para o backend
      try {
        const jwt = await getAuthTokenAsync();
        await fetch('/api/users/google-tokens', {
          method: 'POST',
          headers: {
        ...
        console.error('Falha ao enviar tokens para backend:', err);
      }
  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      if (userData && String(userData.email || '').trim().toLowerCase() === 'admin@duovet.app') {
        userData.role = 'admin';
      }
      setUser(userData);
      setIsAuthenticated(true);
