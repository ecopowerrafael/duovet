...
import { getGlobalSettings, updateGlobalSettings, getUsers, deactivateUser, adminChangeUserPassword, sendNotification, broadcastNotification, activateUserSubscription, getAffiliateAdminPayoutRequests, completeAffiliatePayoutRequest, rejectAffiliatePayoutRequest, getAffiliateAdminUserProfile, setAffiliateCustomCommission, getAffiliateTopAffiliates, getAffiliateConversions } from '../lib/api';
...
export default function AdminPanel() {
  ...
  const [settingsForm, setSettingsForm] = useState({
    ...
    smtp_secure: 'false',
    smtp_from: '',
    affiliateDefaultCommission: '25',
    affiliateMinPayout: '100',
  ...
  const [isAffiliateUserDialogOpen, setIsAffiliateUserDialogOpen] = useState(false);
  const [selectedUserForAffiliate, setSelectedUserForAffiliate] = useState(null);
  const [affiliateUserProfile, setAffiliateUserProfile] = useState(null);
  const [affiliateCommissionInput, setAffiliateCommissionInput] = useState('');
  ...
  const [affiliateTopAffiliates, setAffiliateTopAffiliates] = useState([]);
  ...
  useEffect(() => {
    setSettingsForm({
      ...
      affiliateDefaultCommission: settings?.affiliateDefaultCommission || '25',
      affiliateMinPayout: settings?.affiliateMinPayout || '100',
  ...
  const loadAffiliateData = async () => {
    ...
    try {
      const [payoutRequests, top, conversions] = await Promise.all([
        getAffiliateAdminPayoutRequests('requested'),
        getAffiliateTopAffiliates(10),
        getAffiliateConversions(30)
      ]);
      setAffiliatePayoutRequests(Array.isArray(payoutRequests) ? payoutRequests : []);
      setAffiliateTopAffiliates(Array.isArray(top) ? top : []);
      setAffiliateConversions(Array.isArray(conversions) ? conversions : []);
    } catch (err) {
  ...
  const handleOpenAffiliateUser = async (user) => {
    setSelectedUserForAffiliate(user);
    setAffiliateUserProfile(null);
    setAffiliateCommissionInput('');
    setIsAffiliateUserDialogOpen(true);
    try {
      const profile = await getAffiliateAdminUserProfile(user.id);
      setAffiliateUserProfile(profile);
      setAffiliateCommissionInput(profile?.custom_commission === null || profile?.custom_commission === undefined ? '' : String(profile.custom_commission));
    } catch (err) {
      toast.error('Erro ao carregar dados do afiliado');
    }
  };

  const handleSaveAffiliateUser = async () => {
    if (!selectedUserForAffiliate) return;
    const raw = String(affiliateCommissionInput || '').trim();
    ...
    setLoading(true);
    try {
      await setAffiliateCustomCommission(selectedUserForAffiliate.id, parsed);
      toast.success('Comissão personalizada salva!');
      setIsAffiliateUserDialogOpen(false);
  ...
  const handleCompleteAffiliatePayout = async (id) => {
    setAffiliateLoading(true);
    try {
      await completeAffiliatePayoutRequest(id);
  ...
  const handleRejectAffiliatePayout = async (id) => {
    const note = window.prompt('Motivo (opcional):') || '';
    setAffiliateLoading(true);
    try {
      await rejectAffiliatePayoutRequest(id, note);
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 space-y-6">
        ...
        <CardContent>
          <Tabs defaultValue="login-social">
            ...
            <TabsContent value="smtp">
              <h2 className="text-lg font-semibold mb-4">SMTP (Envio de E-mails)</h2>
              <div className="space-y-4">
                <Input name="smtp_host" value={settingsForm.smtp_host || ''} onChange={handleSettingsChange} placeholder="Servidor SMTP (ex: smtp.gmail.com)" />
                <Input name="smtp_port" value={settingsForm.smtp_port || ''} onChange={handleSettingsChange} placeholder="Porta (ex: 587)" />
                <Input name="smtp_user" value={settingsForm.smtp_user || ''} onChange={handleSettingsChange} placeholder="Usuário (ex: contato@duovet.app)" />
                <Input name="smtp_pass" type="password" value={settingsForm.smtp_pass || ''} onChange={handleSettingsChange} placeholder="Senha (ou senha de app)" />
                <label className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
                  <input
                    type="checkbox"
                    checked={settingsForm.smtp_secure === 'true'}
                    onChange={(e) => setSettingsForm({ ...settingsForm, smtp_secure: e.target.checked ? 'true' : 'false' })}
                  />
                  Conexão segura (SSL/TLS)
                </label>
                <Input name="smtp_from" value={settingsForm.smtp_from || ''} onChange={handleSettingsChange} placeholder="Remetente (ex: DuoVet <contato@duovet.app>)" />
                <Button variant="default" onClick={handleSettingsSave} disabled={loading}>Salvar</Button>
              </div>
            </TabsContent>
            ...
            <TabsContent value="affiliates">
              <h2 className="text-lg font-semibold mb-4">Afiliados</h2>
              <div className="space-y-6">
                <div className="rounded-xl border border-[var(--border-color)] p-4 space-y-3">
                  <h3 className="font-semibold text-[var(--text-primary)]">Configurações Globais</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <Input
                      name="affiliateDefaultCommission"
                      value={settingsForm.affiliateDefaultCommission || ''}
                      onChange={handleSettingsChange}
                      placeholder="Comissão padrão (%)"
                      type="number"
                    />
                    <Input
                      name="affiliateMinPayout"
                      value={settingsForm.affiliateMinPayout || ''}
                      onChange={handleSettingsChange}
                      placeholder="Saque mínimo (R$)"
                      type="number"
                    />
                  </div>
                  <Button variant="default" onClick={handleSettingsSave} disabled={loading}>Salvar</Button>
                ...
                <div className="rounded-xl border border-[var(--border-color)] p-4 space-y-3">
                  <div className="flex items-center justify-between gap-2">
                    <h3 className="font-semibold text-[var(--text-primary)]">Solicitações de Saque</h3>
                    <Button variant="outline" onClick={loadAffiliateData} disabled={affiliateLoading}>Atualizar</Button>
                  </div>
                  <div className="rounded-md border border-[var(--border-color)] overflow-hidden">
                    <Table>
                      ...
                      <TableBody>
                        ...
                        ) : (affiliatePayoutRequests || []).map((row) => (
                          <TableRow key={row.id}>
                            <TableCell>
                              <div className="font-medium">{row.name}</div>
                              <div className="text-sm text-[var(--text-muted)]">{row.email}</div>
                            </TableCell>
                            <TableCell className="max-w-[280px] truncate">{row.pix_key || '-'}</TableCell>
                            <TableCell>{safeFormatDate(row.requested_at)}</TableCell>
                            <TableCell className="text-right">
                              {Number(row.amount || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                            </TableCell>
                            <TableCell className="text-right flex justify-end gap-2">
                              <Button size="sm" onClick={() => handleCompleteAffiliatePayout(row.id)} disabled={affiliateLoading}>
                                Confirmar
                              </Button>
                              <Button variant="outline" size="sm" onClick={() => handleRejectAffiliatePayout(row.id)} disabled={affiliateLoading}>
                                Rejeitar
                              </Button>
                            </TableCell>
                          </TableRow>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  <Card className="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)]">
                    <CardHeader>
                      <CardTitle className="text-base font-bold text-[var(--text-primary)]">Top 10 Afiliados</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="h-[280px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={(affiliateTopAffiliates || []).map((r) => ({ name: r.name, value: Number(r.total_earned || 0) }))}>
                            <CartesianGrid strokeDasharray="3 3" stroke="var(--border-color)" opacity={0.3} />
                            <XAxis dataKey="name" tick={{ fontSize: 11 }} interval={0} tickFormatter={(v) => String(v || '').split(' ')[0]} />
                            <YAxis tick={{ fontSize: 11 }} tickFormatter={(v) => `R$ ${v}`} />
                            <Tooltip formatter={(v) => Number(v || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} />
                  ...
                  <Card className="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)]">
                    <CardHeader>
                      <CardTitle className="text-base font-bold text-[var(--text-primary)]">Volume de Conversão (30 dias)</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="h-[280px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={(affiliateConversions || []).map((r) => ({ date: r.date, value: Number(r.count || 0) }))}>
                            <CartesianGrid strokeDasharray="3 3" stroke="var(--border-color)" opacity={0.3} />
                            <XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={(v) => safeFormatDate(v, 'dd/MM')} />
                            <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
                            <Tooltip labelFormatter={(v) => safeFormatDate(v)} formatter={(v) => [`${v}`, 'Pagantes']} />
                    ...
                    <TableBody>
                      {(filteredUsers || []).length === 0 ? (
                        ...
                        if (u.subscription_end) {
                          ...
                          expirationDate = addDays(createdAt, TRIAL_DAYS);
                        }
                          ...
                          <TableRow key={u.id}>
                            ...
                            <TableCell className="text-right flex justify-end gap-2">
                              ...
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => handleOpenAffiliateUser(u)}
                                disabled={loading}
                              >
                                Afiliado
                              </Button>
      ...
      <Dialog open={isAffiliateUserDialogOpen} onOpenChange={setIsAffiliateUserDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Afiliado - Comissão Personalizada</DialogTitle>
            <DialogDescription>
              {selectedUserForAffiliate ? `${selectedUserForAffiliate.name} (${selectedUserForAffiliate.email})` : ''}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <Input
              value={affiliateCommissionInput}
              onChange={(e) => setAffiliateCommissionInput(e.target.value)}
              placeholder="Ex: 25"
              type="number"
            />
            <div className="text-xs text-[var(--text-muted)]">
              {affiliateUserProfile ? `Total ganho: R$ ${Number(affiliateUserProfile.total_earned || 0).toFixed(2)}` : ''}
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsAffiliateUserDialogOpen(false)}>Cancelar</Button>
            <Button onClick={handleSaveAffiliateUser} disabled={loading}>Salvar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
