...
export default function ProfileSettings() {
  ...
  const isAdmin = String(user?.email || '').trim().toLowerCase() === 'admin@duovet.app' || user?.role === 'admin';
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="hidden md:block">
          ...
          <TabsList className={`grid w-full bg-[var(--bg-card)] h-auto rounded-xl p-1 gap-1 ${isAdmin ? 'grid-cols-10' : 'grid-cols-9'}`}>
            {tabs.map((tab) => (
              <TabsTrigger 
                key={tab.value} 
                value={tab.value} 
                    ...
                    <div>
                      ...
                      <div className="flex flex-col items-center gap-4">
                        <div className="w-32 h-32 bg-[var(--bg-tertiary)] rounded-2xl flex items-center justify-center overflow-hidden border-2 border-[var(--border-color)]">
                          ...
                          {/* Fallback logo */}
                          <Building2 className="w-12 h-12 text-[var(--text-muted)] logo-fallback" style={{display: formData.logo_url ? 'none' : 'block'}} />
                        </div>
                        <input
                          type="file"
                          id="logo"
                    ...
                    <div>
                      ...
                      <div className="flex flex-col items-center gap-4">
                        <div className="w-32 h-32 bg-[var(--bg-tertiary)] rounded-2xl flex items-center justify-center overflow-hidden border-2 border-[var(--border-color)]">
                          ...
                          {/* Fallback foto */}
                          <User className="w-12 h-12 text-[var(--text-muted)] photo-fallback" style={{display: formData.photo_url ? 'none' : 'block'}} />
                        </div>
                        <input
                          type="file"
                          id="photo"
          ...
          <TabsContent value="professional" className="mt-6 space-y-6">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <Card className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl">
                <CardHeader>
                  <CardTitle className="text-lg md:text-xl text-[var(--text-primary)] leading-tight">Assinatura Digital</CardTitle>
                  <CardDescription className="text-xs md:text-sm text-[var(--text-muted)] leading-tight">
                    Upload da sua assinatura para documentos e relatórios
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="flex flex-col items-center gap-2 md:gap-4">
                    <div className="w-full max-w-sm h-24 md:h-32 bg-[var(--bg-tertiary)] rounded-xl flex items-center justify-center overflow-hidden border-2 border-[var(--border-color)]">
                      {formData.signature_url ? (
                        <img src={formData.signature_url} alt="Assinatura" className="max-h-full object-contain" />
                      ) : (
                        <ImageIcon className="w-8 h-8 md:w-12 md:h-12 text-[var(--text-muted)]" />
                      )}
                    </div>
                    <input
                      type="file"
                    ...
                    <label htmlFor="signature">
                      <Button type="button" variant="outline" asChild disabled={isUploading} className="rounded-xl text-xs md:text-base px-3 md:px-4 py-1 md:py-2">
