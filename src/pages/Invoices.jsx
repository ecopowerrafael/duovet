...
export default function Invoices() {
  ...
  const [showFiscalKeyWarning, setShowFiscalKeyWarning] = useState(false);
  const handleEmitNF = (payment) => {
    // Verifica se o usuário tem chave fiscal preenchida
    if (!user?.fiscal_api_key) {
      setShowFiscalKeyWarning(true);
      return;
    }
  ...
  const handleMonthlyExport = async () => {
    ...
    try {
      ...
      const reportData = invoices.map(invoice => ({
        'Data': formatDate(invoice.date, 'dd/MM/yyyy'),
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Dialog: Aviso de chave fiscal */}
      <Dialog open={showFiscalKeyWarning} onOpenChange={setShowFiscalKeyWarning}>
        <DialogContent className="sm:max-w-lg rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-[var(--text-primary)]">Você precisa configurar suas chaves fiscais</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="p-4 bg-amber-500/10 rounded-xl border border-amber-200">
              <div className="flex gap-3">
                ...
                <div className="text-sm text-amber-900">
                  <p className="font-semibold mb-1">Configuração obrigatória</p>
                  <p>Para emitir Nota Fiscal, é necessário configurar sua chave de API fiscal.</p>
                </div>
              </div>
            </div>
            <div className="flex gap-3 pt-2">
              <Button
                className="flex-1 h-11 rounded-xl bg-white border border-[#22c55e] hover:bg-[#22c55e] text-black dark:bg-slate-800 dark:text-slate-100 dark:hover:bg-[#22c55e]/90"
                onClick={() => {
                  setShowFiscalKeyWarning(false);
                  window.location.href = '/#/settings?tab=fiscal';
                }}
              >
                Ir para Perfil
              </Button>
              <Button
                variant="outline"
                className="flex-1 h-11 rounded-xl bg-white border border-black text-black hover:bg-gray-100 dark:bg-slate-800 dark:border-slate-600 dark:text-slate-100 dark:hover:bg-slate-700"
                onClick={() => setShowFiscalKeyWarning(false)}
              >
                Fechar
              </Button>
            </div>
