  ...
  const generatePDF = async (includePix = false, includePrescription = false, includeReproductive = false, options = {}) => {
    ...
    if (resolvedTotalAmount > 0) {
      ...
      if (resolvedMedicationsTotal > 0) {
        ...
        y += 5;
        // Lista de valores individuais dos medicamentos
        if (Array.isArray(appointment.medications) && appointment.medications.length > 0) {
          doc.setFontSize(8);
          doc.setFont(undefined, 'normal');
          doc.setTextColor(80, 80, 80);
          appointment.medications.forEach((med, idx) => {
            const unitPrice = Number(med?.price ?? med?.value) || 0;
            const quantity = Number(med?.quantity) || 1;
            const total = unitPrice * quantity;
            const medLine = `${idx + 1}. ${med.name} - Qtd: ${quantity} | Unit: R$ ${unitPrice.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} | Total: R$ ${total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`;
            doc.text(medLine, 18, y);
            y += 4;
          });
          doc.setFontSize(9);
          doc.setFont(undefined, 'normal');
    ...
    // Usa blob URL explícito para garantir download em WebView (Capacitor) e navegadores modernos
