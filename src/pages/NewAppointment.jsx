...
export default function NewAppointment() {
  ...
  const toCentsString = (value) => Math.round((Number(value) || 0) * 100).toString();
  const calculatedDisplacementCost = roundMoney(selectedProperty?.distance_km ? (selectedProperty.distance_km * 2 * kmRate) : 0);
  const displacementCost = formData.custom_displacement_cost !== null
    ? roundMoney(formData.custom_displacement_cost)
    : calculatedDisplacementCost;
  const totalAmount = roundMoney(totalProcedures + totalMedications + displacementCost);
            ...
            {(calculatedDisplacementCost > 0 || formData.custom_displacement_cost !== null) && (
              <div>
                ...
                <div className="flex items-center gap-2">
                  <Input
                    value={formatCurrency(toCentsString(formData.custom_displacement_cost !== null ? formData.custom_displacement_cost : calculatedDisplacementCost))}
                    onChange={(e) => {
                ...
                {formData.custom_displacement_cost !== null && Math.round(Number(formData.custom_displacement_cost) * 100) !== Math.round(calculatedDisplacementCost * 100) && (
