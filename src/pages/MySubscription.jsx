...
export default function MySubscription() {
  ...
  if (isAuthenticated) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50/20 dark:from-[#0b0b0f] dark:via-[#111318] dark:to-[#0f1b16] py-6 md:py-12 px-4 overflow-x-hidden">
        <div className="max-w-4xl mx-auto space-y-4 md:space-y-8 w-full">
          <div className="text-center mb-4 md:mb-12">
            <h1 className="text-2xl md:text-4xl font-black text-slate-900 dark:text-slate-50 mb-1 md:mb-2 tracking-tight">Minha Assinatura</h1>
            <p className="text-sm md:text-lg text-slate-500 dark:text-slate-300 font-medium">Gerencie sua conta DuoVet {currentPlanName}</p>
          </div>

          <div className="flex items-center justify-center gap-2 md:gap-3 mb-4 md:mb-8">
            <button
              onClick={() => setBillingPeriod('monthly')}
              className={`px-4 md:px-6 py-1.5 md:py-2 rounded-xl text-sm md:text-base font-bold transition-all ${
            ...
            <button
              ...
              className={`px-4 md:px-6 py-1.5 md:py-2 rounded-xl text-sm md:text-base font-bold transition-all relative ${
            ...
            >
              Anual
              <Badge className="ml-1 md:ml-2 bg-amber-500 text-white text-[9px] md:text-[10px]">Economize 20%</Badge>
            </button>
          </div>
          ...
          {isTrial && daysLeft > 0 && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <Card className="bg-gradient-to-br from-emerald-600 to-teal-600 border-0 shadow-2xl shadow-emerald-500/20 rounded-2xl md:rounded-[2.5rem] text-white overflow-hidden relative">
                <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-32 -mt-32 blur-3xl" />
                <CardContent className="p-4 md:p-10 relative z-10">
                  <div className="flex flex-row items-center gap-4 md:gap-8">
                    <div className="w-12 h-12 md:w-20 md:h-20 bg-white/20 backdrop-blur-md rounded-2xl md:rounded-3xl flex items-center justify-center flex-shrink-0 shadow-inner">
                      <Sparkles className="w-6 h-6 md:w-10 md:h-10 text-white animate-pulse" />
                    </div>
                    <div className="flex-1 text-left">
                      <h2 className="text-lg md:text-3xl font-black mb-1 md:mb-3">
                        Teste Gratuito Ativo
                      </h2>
                      <p className="text-sm md:text-xl text-emerald-50/90 mb-2 md:mb-6 font-medium">
                        Você tem <span className="text-white font-black underline decoration-2 underline-offset-4">{daysLeft} {daysLeft === 1 ? 'dia' : 'dias'}</span> restantes.
                      </p>
                      <div className="bg-black/10 backdrop-blur-sm rounded-xl md:rounded-2xl p-2 md:p-5 border border-white/10 inline-block text-left">
                        <div className="flex items-center gap-2 md:gap-3">
                          <CreditCard className="w-4 h-4 md:w-5 md:h-5 text-emerald-200" />
                          <p className="text-[10px] md:text-sm font-bold text-white leading-tight">
                            Fim em: {format(trialEndDate, "d 'de' MMM", { locale: ptBR })}
                          </p>
                        </div>
                      </div>
                    </div>
          ...
          {isCanceled && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <Card className="bg-gradient-to-br from-amber-500 to-orange-500 border-0 shadow-2xl shadow-amber-500/20 rounded-2xl md:rounded-[2.5rem] text-white overflow-hidden relative">
                <CardContent className="p-4 md:p-10 relative z-10">
                  <div className="flex flex-row items-center gap-4 md:gap-8">
                    <div className="w-12 h-12 md:w-20 md:h-20 bg-white/20 backdrop-blur-md rounded-2xl md:rounded-3xl flex items-center justify-center flex-shrink-0 shadow-inner">
                    ...
                    <div className="flex-1 text-left">
                      <h2 className="text-lg md:text-3xl font-black mb-1 md:mb-3">Assinatura Cancelada</h2>
                      <p className="text-xs md:text-xl text-amber-50/90 mb-2 md:mb-6 font-medium">
                        Acesso até {subscription.end_date ? format(new Date(subscription.end_date), "d/MM", { locale: ptBR }) : 'fim do ciclo'}.
                      </p>
                      <Button 
                        onClick={() => window.location.href = createPageUrl('Plans')}
                        className="bg-white text-amber-600 hover:bg-amber-50 font-black rounded-lg md:rounded-xl h-8 md:h-12 px-4 md:px-8 text-xs md:text-base"
                      >
                        REATIVAR
                      </Button>
                    </div>
          ...
          {!subscription && !isLoading && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <Card className="bg-gradient-to-br from-slate-800 to-slate-900 border-0 shadow-2xl shadow-slate-900/20 rounded-2xl md:rounded-[2.5rem] text-white overflow-hidden relative">
                <CardContent className="p-6 md:p-10 relative z-10 text-center">
                  <Zap className="w-10 h-10 md:w-16 md:h-16 text-emerald-400 mx-auto mb-4 md:mb-6 animate-bounce" />
                  <h2 className="text-xl md:text-3xl font-black mb-2 md:mb-3">Sem plano ativo</h2>
                  <p className="text-sm md:text-xl text-slate-400 mb-6 md:mb-8 max-w-lg mx-auto">
                    Libere acesso ilimitado a todas as ferramentas agora.
                  </p>
                  <Button 
                    onClick={() => handleCheckout(fallbackPlan?.id || 'autonomo', billingPeriod)}
                    disabled={isProcessing || !fallbackPlan}
                    className="bg-emerald-500 hover:bg-emerald-600 text-white font-black rounded-xl md:rounded-2xl h-10 md:h-14 px-8 md:px-12 text-sm md:text-lg shadow-xl shadow-emerald-500/30"
                  >
                    {fallbackPlan ? 'ASSINAR AGORA' : 'INDISPONÍVEL'}
                  </Button>
                </CardContent>
              </Card>
            </motion.div>
          ...
          {(isActive || isTrial || isCanceled) && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
              <Card className="md:col-span-2 shadow-xl shadow-slate-200/50 dark:shadow-black/30 border-0 rounded-2xl md:rounded-[2rem] bg-white dark:bg-[#151922] overflow-hidden">
                <CardContent className="p-4 md:p-8">
                  <div className="flex items-center justify-between mb-4 md:mb-8">
                    <div className="flex items-center gap-3 md:gap-5">
                      <div className="w-12 h-12 md:w-16 md:h-16 bg-emerald-50 dark:bg-emerald-500/10 rounded-xl md:rounded-2xl flex items-center justify-center shadow-inner">
                        <Crown className="w-6 h-6 md:w-8 md:h-8 text-emerald-600" />
                      </div>
                      <div>
                        <h3 className="text-lg md:text-2xl font-black text-slate-900 dark:text-slate-50 leading-tight">DuoVet {currentPlanName}</h3>
                        <Badge className={`border-0 px-2 md:px-3 py-0.5 md:py-1 text-[9px] md:text-xs font-black uppercase tracking-widest mt-0.5 md:mt-1 ${
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 md:gap-8 py-4 md:py-8 border-y border-slate-50 dark:border-slate-800">
                    <div className="hidden sm:block">
                      <p className="text-[9px] md:text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1 md:mb-2">Cadastro</p>
                      <p className="text-sm md:text-xl font-black text-slate-900 dark:text-slate-50">
                        {createdAt ? format(createdAt, "d 'de' MMM", { locale: ptBR }) : '-'}
                      </p>
                    </div>
                    <div>
                      <p className="text-[9px] md:text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1 md:mb-2">Valor Mensal</p>
                      <p className="text-xl md:text-3xl font-black text-slate-900 dark:text-slate-50">
                        R$ {toSafeNumber(currentPrice).toFixed(2).replace('.', ',')}
                      </p>
                    </div>
                    <div>
                      <p className="text-[9px] md:text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1 md:mb-2 text-right sm:text-left">
                        {isTrial ? `Fim (${daysLeft}d)` : isCanceled ? 'Acesso até' : 'Próxima Cobrança'}
                      </p>
                      <p className={`text-xl md:text-3xl font-black text-right sm:text-left ${isTrial ? 'text-amber-500' : 'text-slate-900 dark:text-slate-50'}`}>
                  ...
                  <div className="mt-4 md:mt-8 grid grid-cols-1 sm:grid-cols-3 gap-2 md:gap-4">
                    {availablePlans.map((plan) => (
                      <Button 
                        key={plan.id}
                        variant={subscription?.plan === plan.id ? "secondary" : "outline"}
                        className={`rounded-xl md:rounded-2xl border-slate-200 font-bold h-10 md:h-12 px-3 md:px-4 text-[10px] md:text-xs ${
                  ...
                  {!isCanceled && (
                    <div className="mt-4 md:mt-6 pt-4 md:pt-6 border-t border-slate-50">
                      <Button 
                        variant="ghost" 
                        className="rounded-xl md:rounded-2xl text-slate-400 dark:text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30 font-bold h-8 md:h-10 px-4 md:px-6 transition-colors text-[10px] md:text-xs"
                        onClick={() => {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-1 gap-4 md:space-y-6">
                <Card className="shadow-xl shadow-slate-200/50 dark:shadow-black/30 border-0 rounded-2xl md:rounded-[2rem] bg-white dark:bg-[#151922] overflow-hidden">
                  <CardContent className="p-4 md:p-6">
                    <div className="flex items-center gap-3 mb-4 md:mb-6">
                      <div className="w-10 h-10 md:w-12 md:h-12 bg-emerald-50 dark:bg-emerald-500/10 rounded-xl md:rounded-2xl flex items-center justify-center">
                        <Calendar className="w-5 h-5 md:w-6 md:h-6 text-emerald-600" />
                      </div>
                      <div>
                        <p className="text-[9px] md:text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                          {isTrial ? 'Tempo restante' : isCanceled ? 'Acesso restante' : 'Renovação'}
                        </p>
                        <h3 className="text-sm md:text-lg font-black text-slate-900 dark:text-slate-50">
                          {countdown.days}d {countdown.hours}h restantes
                        </h3>
                      </div>
                    </div>

                    <div className="mb-2 flex items-center justify-between text-[9px] md:text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">
                      <span>{usagePercent}% utilizado</span>
                    </div>
                    <div className="h-2 md:h-3 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden mb-2 md:mb-4">
                      <div
                ...
                <Card className="shadow-xl shadow-slate-200/50 dark:shadow-black/30 border-0 rounded-2xl md:rounded-[2rem] bg-white dark:bg-[#151922] overflow-hidden">
                  <CardContent className="p-4 md:p-6 space-y-3 md:space-y-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 md:w-12 md:h-12 bg-slate-100 dark:bg-slate-800 rounded-xl md:rounded-2xl flex items-center justify-center">
                        <HelpCircle className="w-5 h-5 md:w-6 md:h-6 text-slate-700" />
                      </div>
                      <h3 className="text-sm md:text-lg font-black text-slate-900 dark:text-slate-50">Gerenciar</h3>
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                      <Button
                        ...
                        className="w-full rounded-xl md:rounded-2xl h-10 md:h-12 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs md:text-sm"
                      >
                        {isProcessing ? '...' : isActive ? 'Pagamento' : 'Assinar'}
                      </Button>
                      ...
                      <Button
                        ...
                        className="w-full rounded-xl md:rounded-2xl h-10 md:h-12 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800 font-bold text-xs md:text-sm"
                      >
                        Suporte
                      </Button>
                    </div>
          ...
          {(paymentHistory && paymentHistory.length > 0) && (
            <Card className="shadow-xl shadow-slate-200/50 dark:shadow-black/30 border-0 rounded-2xl md:rounded-[2rem] bg-white dark:bg-[#151922]">
              <CardContent className="p-4 md:p-8">
                <h3 className="text-lg md:text-xl font-black text-slate-900 dark:text-slate-50 mb-4 md:mb-8 flex items-center gap-2 md:gap-3">
                ...
                <div className="space-y-3 md:space-y-4">
                  {paymentHistory.map((payment) => (
                    <div key={payment.id} className="flex items-center justify-between p-3 md:p-5 bg-slate-50 dark:bg-slate-800/60 rounded-xl md:rounded-2xl border border-slate-100 dark:border-slate-800 group hover:border-emerald-200 dark:hover:border-emerald-800 transition-colors">
                      <div className="flex items-center gap-3 md:gap-5">
                        <div className="w-10 h-10 md:w-12 md:h-12 bg-white dark:bg-slate-900 rounded-lg md:rounded-xl flex items-center justify-center border border-slate-100 dark:border-slate-800 shadow-sm group-hover:scale-110 transition-transform">
                          <CreditCard className="w-5 h-5 md:w-6 md:h-6 text-slate-400" />
                        </div>
                        <div>
                          <p className="font-black text-slate-900 dark:text-slate-50 text-base md:text-lg">
                          ...
                          <p className="text-[9px] md:text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                            {payment.date ? format(new Date(payment.date), "d 'de' MMM", { locale: ptBR }) : '-'}
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <Badge className="bg-emerald-100 text-emerald-700 border-0 mb-1 font-black px-2 md:px-3 py-0.5 md:py-1 rounded-lg text-[9px] md:text-[10px] uppercase tracking-widest">
                          PAGO
                        </Badge>
                        <p className="text-[9px] md:text-[10px] font-bold text-slate-400 dark:text-slate-500">{payment.method || 'Cartão'}</p>
                      </div>
                    </div>
                  ))}
                </div>
